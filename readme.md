...
- PNPM `pnpm-lock.yaml` lock files with `lockfileVersion` value `6` or `9`. In practice, these lock files are generated by PNPM versions 8 and newer (and technically version 7 as well, but opt-in was required).
  - Lock files with a leading PNPM version document (written by PNPM 11 and newer when PNPM is pinned via `devEngines.packageManager`) are supported; the pin is not reported as a dependency.
...
2. Yarn lock files do not distinguish `prod` and `dev` direct dependencies ([reference](https://github.com/yarnpkg/berry/blob/afc93bbbe7e17f1469b9b73e3a303a1dd2c7aff0/packages/yarnpkg-core/sources/types.ts#L142-L147)). They are merged into a single bucket. As a result, `--include prod` and `--include dev` produce identical output, and `--omit dev` has no effect. Peer-only direct dependencies are not surfaced because Yarn does not install them. Only resolved, installed packages appear in diffs.
